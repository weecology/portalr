...
make_timeseries = function(data, date_format="%m/%d/%Y"){
  ...
  data = calc_diff(data)
  data$values = na.approx(data$values) # default is linear interpolation
  
  
  # Repeated samples in a month are averaged 
  data = data %>% group_by(newdate) %>% summarise(values = mean(values))
  return(data)
}
...
filter_diffs = function(data, diff_value){
  ...
  subset = data %>% filter(mo_diff == diff_value) %>% select(period)
...
fix_period = function(data, i_period, adj){
  ...
  modify = data %>% filter(period==i_period) %>%
    mutate(newdate = newdate %m+% months(adj))
  data <- data %>% mutate(newdate = replace(newdate, period == i_period,
                                            modify$newdate))
}

insert_missing_months = function(data, i_period){
  gap_begin = data %>% filter(period == i_period)
  counter = gap_begin$mo_diff-1
  new_records = c()
  for (i in 1:counter){
    new_record = gap_begin %>% mutate(newdate = newdate %m+% months(i)) 
