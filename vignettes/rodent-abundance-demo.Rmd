...
The first table that we loaded (`data_tables$rodent_data`) is a record of the macrofauna observed, including rodents, but also other taxa. If we just wanted to get the rodent abundance data, we could use the `abundance` function. However, going through the all of the data filtering steps helps us to get a better of what we actually end up with.

```{r}
rodent_data <- clean_rodent_data(data_tables)
...
summarize_abundance <- function(rodent_data)
{
  ...
  return(rodent_data %>%
           ...
           select(period, species, abundance = n)
  )
}
