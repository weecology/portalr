...
The `loadData` function reads in several tables related to the rodent abundances. We won't necessarily use all of these tables, but loading and naming them now will make it easier later.

```{r}
rodent_data_all <- loadData(path = my_path)
rodent_data <- rodent_data_all[[1]]
species_table <- rodent_data_all[[2]]
trapping_table <- rodent_data_all[[3]]
newmoons_table <- rodent_data_all[[4]]
plots_table <- rodent_data_all[[5]]
```
