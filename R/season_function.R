...
#'
#' @examples add_seasons(abundance(path = '.',time="newmoon"), date_column = "newmoonnumber", season_level = 2, summarize = "mean")
#'
#' @export
#'
add_seasons <- function(data, level = "site", season_level = 2, date_column = yearmon, summarize = NA, path = "~"){
  ...
  if(date_column == "period"){
    full_data = data %>% dplyr::left_join(data_tables$newmoons_table,by="period")  %>%
      dplyr::mutate(year = lubridate::year(censusdate), month = lubridate::month(censusdate)) %>%
      dplyr::select(-newmoonnumber,-newmoondate, -censusdate) %>% dplyr::group_by(year,month)

    } else if(date_column == "newmoonnumber"){
    full_data = data %>% dplyr::left_join(data_tables$newmoons_table,by="newmoonnumber")  %>%
      dplyr::mutate(year = lubridate::year(censusdate), month = lubridate::month(censusdate)) %>%
      dplyr::select(-newmoondate, -censusdate, -period) %>% dplyr::group_by(year,month)
...
#'
#' @examples yearly(abundance(path = '.',time="newmoon"), date_column = "newmoonnumber")
#'
#' @export
#'
yearly <- function(...) {
