...
#' @export
#'
colony_presence_absence= function(path = '~', level='Site', rare_sp = F, unknowns = F) {
  ...
  # if rare_sp == F, the species list will be restricted to only the 8 species we absolutely know were censused consistently
  if (unknowns == F) {
    specieslist = antsp$speciescode[!(antsp$speciescode %in% c('unkn','camp sp','phei sp','novo sp'))]
  }
  if (rare_sp == F) {
...
bait_presence_absence= function(path = '~', level='Site') {
  ...
  if (level == 'Stake') {
    ...
    #data frame of which plots were censused in which years
    full_df = expand.grid(year = unique(bait$year), plot = seq(24), stake = c(11,13,15,17,
                                                                              22,24,26,
                                                                              31,33,35,37,
                                                                              42,44,46,
                                                                              51,53,55,57,
