...
weather <- function(level, path = '~') {
  ...
  weather_new=read.csv(file.path(path,'PortalData/Weather/Portal_weather.csv'), na.strings=c(""), stringsAsFactors = FALSE)
  weather_old=read.csv(file.path(path,'PortalData/Weather/Portal_weather_19801989.csv'), na.strings=c("-99"), stringsAsFactors = FALSE)
  NDVI=read.csv(file.path(path,'PortalData/NDVI/monthly_NDVI.csv'), na.strings=c("-99"), stringsAsFactors = FALSE)
