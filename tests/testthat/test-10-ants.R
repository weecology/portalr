...
test_that("colony_presence_absence returns expected results", {
  colonypresabs.stake = colony_presence_absence(path = '.', level = "Stake", rare_sp = T, unknowns = T)

  test.colony = dplyr::filter(colonypresabs.stake, year %in% 2000:2005)
  expect_true(sum(test.colony$presence, na.rm = T) == 4930)
  expect_false(anyNA(test.colony))
  expect_equal(nrow(test.colony), 134064)

  colonypresabs.site = colony_presence_absence(path = '.', level = "Site", rare_sp = T, unknowns = T)
...
test_that("bait_presence_absence returns expected results", {
  baitpresabs.stake = bait_presence_absence(path = '.', level = "Stake")
  ...
  baitpresabs.site = bait_presence_absence(path = '.', level = "Site")
