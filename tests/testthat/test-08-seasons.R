...
test_weather = weather("monthly",path=".") %>% dplyr::filter(year %in% 1989:2010)

test_that("add_seasons returns expected results", {

  rodent_seasons = add_seasons(test_rodents,level = "treatment", date_column = "period",season_level=2, path = ".")
  expect_true(expect_true(sum(rodent_seasons$abundance, na.rm = T) == 16452))

  rodent_seasons = add_seasons(test_rodents,level = "treatment", date_column = "period",season_level=2,summarize="mean", path = ".")
  expect_true(expect_true(round(sum(rodent_seasons$abundance, na.rm = T),4) == 478.3957))

  weather_seasons = add_seasons(test_weather,date_column = "yearmon",season_level = 4, summarize = "mean", path = ".")
...
test_that("yearly returns expected results", {
  ...
  rodent_yearly = yearly(test_rodents,level = "plot", date_column = "period", path = ".")
  expect_true(expect_true(round(sum(rodent_yearly$abundance, na.rm = T),3) == 1533.19))

  weather_yearly = yearly(test_weather,date_column = "yearmon", path = ".")
